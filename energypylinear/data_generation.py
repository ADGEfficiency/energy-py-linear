...
def generate_random_ev_input_data(
    ...
    electricity_carbon_intensities = np.random.normal(
        intensity_mu, intensity_std, idx_length
    )
    ...
    for step in range(charge_events.shape[0]):
        length = int(np.random.randint(1, charge_length))
        start = int(np.random.randint(0, charge_events.shape[1] - length - 1))
    ...
    return {
        ...
        "electricity_carbon_intensities": electricity_carbon_intensities.tolist(),
