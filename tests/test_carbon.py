...
@pytest.mark.parametrize(
    ...
    [([20, 10, 10], 1, [10, 20, 20]), ([10, 50, 10, 50, 10], 0, [50, 10, 50, 10, 50])],
)
def test_carbon_optimization(prices, initial_charge, carbon):
    power = 4
    capacity = 6

    model = energypylinear.Battery(power=power, capacity=capacity, efficiency=1.0)

    p_info = model.optimize(prices=prices, initial_charge=initial_charge, freq="60T")
